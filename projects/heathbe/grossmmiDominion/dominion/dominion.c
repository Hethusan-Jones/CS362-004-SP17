...
int adventurerM(int currentPlayer, int *cardDrawn, int *drawntreasure, int* temphand, int* z, struct gameState *state)
{
	int temp = 0;

  while( (*drawntreasure<2) && (*z < 500) ){
...
int villageM(int currentPlayer, int handPos, struct gameState *state)
{
  //+1 Card
  drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  /*    while(drawntreasure<2){
    ...
    */
    ...
    case council_room:
      //+4 Cards
	  return council_roomM(state, handPos);
...
int council_roomM(struct gameState *state, int handPos) {
	int currentPlayer = whoseTurn(state);
	int i;
      //+4 Cards
      for (i = 0; i < 4; i++)
